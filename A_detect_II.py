...
def detect_circles_color(img_color, img_canny,dp, center_dist, param2, minR, maxR,tolerancia=50,tolerancia_int=1):
    image=img_canny.copy()
    # Get the dimensions of the input image
    h, w = image.shape
...
def detect_circles_final(img_color,img_canny,dp, center_dist,param2,minR, maxR,tolerancia,tolerancia_int):
    ...
    first=detect_circles_color(img_color,img_canny,dp,center_dist,param2=0.4*p2,minR=one,maxR=two,tolerancia=50,tolerancia_int=1)
    second=detect_circles_color(img_color,img_canny,dp,center_dist,param2=0.8*p2,minR=two,maxR=tree,tolerancia=35,tolerancia_int=1)
    third=detect_circles_color(img_color,img_canny,dp,center_dist,param2=p2,minR=tree,maxR=four,tolerancia=10,tolerancia_int=1)
    small=first[3]
    small_image=first[0]
